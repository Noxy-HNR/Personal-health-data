    ...
    for name in names:
        try:
            ...
            for record in records:
                ...
                if day:
                    by_day.setdefault(day, {})[name] = _pick(record, allowed)
...
def _compact_result(period: dict[str, Any], daily: list[dict[str, Any]], errors: dict[str, str]) -> dict[str, Any]:
    # Keep errors compact too; authentication/API diagnostics can otherwise
    # consume more context than the actual health data.
...
def get_health_snapshot_compact(days: int = 2) -> dict[str, Any]:
    ...
    Defaults to only the latest two days and four high-value collections. It
    deliberately excludes contributor maps, workouts, tags, resilience, SpO2,
    cardiovascular age, VO2 max and all raw time-series data.
    """
...
@mcp.tool(
    ...
    description=(
        "Return a VERY small Oura health snapshot for broad questions. "
        "Defaults to the latest 2 days and only sleep, readiness, activity and stress. "
        "Never returns raw time-series arrays or verbose API metadata."
    ),
)
...
@mcp.tool(
    ...
    description=(
        "Return a compact aggregate of recent Oura activity, sleep, readiness and stress. "
        "It never returns raw records, minute-by-minute arrays, contributor maps, workouts or tags."
    ),
)
def get_recent_activity_and_recovery(days: int = 7) -> dict[str, Any]:
    ...
    for collection, field in (
        ...
        values = [
            row.get(collection, {}).get(field)
            for row in daily
            if isinstance(row.get(collection), dict)
        ]
        values = [float(v) for v in values if isinstance(v, (int, float))]
        if values:
            averages[f"{collection}.{field}"] = round(mean(values), 1)
