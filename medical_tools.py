...
def register_medical_tools(mcp):
    ...
    def search_loinc(query:str,max_results:int=10)->dict[str,Any]:
        ...
        p=ctsearch("loinc_items",query,max_results,"LOINC_NUM,text","LOINC_NUM,text"); rows=p[3] if len(p)>3 else []
        return {"query":query,"total":p[0] if p else 0,"results":[{"loinc":r[0],"name":r[1] if len(r)>1 else ""} for r in rows],"source":"LOINC / NLM Clinical Tables"}
    @mcp.tool()
